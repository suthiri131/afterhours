...
exports.showWatchList = async (req, res) => {
  try {
    ...
    if (req.query.msg === "deleted")
      msg = "Movie has been removed from your watchlist.";
    if (req.query.msg === "watched")
      msg = "Movie status has been updated to: Watched";
    ...
    const moviesWithReviewStatus = await Promise.all(
      cleanMovies.map(async (item) => {
        const review = await Review.findByMovieIdAndUserId(
          item.movieId._id,
          userId,
        );
        return {
